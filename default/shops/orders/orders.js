    ...
    $.post(WST.U('home/orders/getMoneyByOrder'), { id: id }, function(data) {
        ...
        if (json.status > 0 && json.data) {
            ...
            WST.open({
                ...
                shade: [0.6, '#000'],
                border: [0],
                content: $('#editMoneyBox'),
                area: ['550px', '320px'],
                btn: ['确定', '取消'],
                yes: function(index, layero) {
                    var newOrderMoney = $('#m_newOrderMoney').val();
                    WST.confirm({
                        content: '您确定修改后的订单价格为¥<span class="j-warn-order-money">' + newOrderMoney + '</span>吗？',
                        yes: function(cf) {
...
function deliver(id, deliverType) {
    if (deliverType == 1) {
        ...
        WST.open({
            ...
            btn: ['确定发货', '取消'],
            yes: function(index, layero) {
                var expressgs = $('#expressId').val();
                var express = $("#expressNo").val();
                var ver = /^[0-9a-zA-Z]{8,20}$/g;
                if (expressgs == "") {
                    WST.msg('请填写快递公司！', { icon: 3 });
                } else if (express == "") {
                    WST.msg('请填写快递号！', { icon: 3 });
                } else if (!ver.test(express)) {
                    WST.msg('请填写正确的快递号！', { icon: 3 });
                } else {
...
function modify(th) {
    ...
    layer.open({
        ...
        btn: ['修改', '取消'],
        yes: function(index, layero) {
            var expressNo1 = $("#expressNo1").val();
            var expressId1 = $("#expressId1").val();
            var aa = /^[0-9a-zA-Z]{8,20}$/g;
            var url = location.href;
            var id = url.split('?id=')[1];
            if (!aa.test(expressNo1)) {
                WST.msg('请填写正确的快递号！', { icon: 3 });
            } else {
                var ll = WST.load({ msg: '正在提交信息，请稍候...' });
                $.get(WST.U('home/orders/editOrderExpressNo'), { orderId: id, expressNo: expressNo1, expressCompanyId: expressId1 }, function(data) {
                    var json = WST.toJson(data);    
                    console.log(json);               
                    if (json == 1) {
                        location.reload();
