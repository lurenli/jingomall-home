        ...
        callback: function(no) {
            ...
            if (!initBatchUpload && no == 2) {
                ...
                var uploader = batchUpload({
                    ...
                    uploadServer: WST.U('home/index/uploadPic'),
    ...
    WST.upload({
        ...
        callback: function(f) {
            ...
            if (json.status == 1) {
                ...
                $('#preview').attr('src', "http://jingomall.com.cn/" + json.savePath + json.thumb);
...
function toEdit(id, src) {
    var pageNum = document.querySelector('.laypage_curr');
    pageNum = pageNum ? pageNum.innerHTML : '';
    //下拉框内的值 value
    var index = document.getElementById("cat1").selectedIndex;
    cat1 = document.getElementById("cat1").options[index].value;

    var index = document.getElementById("cat2").selectedIndex;
    cat2Name = document.getElementById("cat2").options[index].text;
    cat2 = document.getElementById("cat2").options[index].value;

    var index = document.getElementById("goodsType").selectedIndex;
    goodsType = document.getElementById("goodsType").options[index].value;
    // input内的value
    var goodsName = document.getElementById("goodsName").value;
    //原代码
    //location.href = WST.U('home/goods/edit','id='+id+'&src='+src);
    location.href = WST.U('home/goods/edit', 'id=' + id + '&src=' + src + '&pageNum=' + pageNum + '&cat1=' + cat1 + '&cat2Name=' + cat2Name + '&goodsType1=' + goodsType + '&keyWord=' + goodsName + '&cat2Id=' + cat2);
            ...
            $.post(WST.U('home/goods/' + ((params.goodsId == 0) ? "toAdd" : "toEdit")), params, function(data, textStatus) {
                ...
                if (json.status == '1') {
                    ...
                    if (params.goodsType == 1) {
                        ...
                        // location.href=WST.U('home/goods/sale2','pageNum='+params.pageNum+'&keyWord='+params.keyWord+'&cat1='+params.cat1+'&goodsType1='+params.goodsType1+'&cat2Id='+params.cat2Id+'&cat2Name='+params.cat2Name+'&status='+params.status);
                        //location.href=WST.U('home/goods/'+src);原代码
                        // location.href = WST.U('home/goods/sale2', 'pageNum=' + params.pageNum + '&keyWord=' + params.keyWord + '&cat1=' + params.cat1 + '&goodsType1=' + params.goodsType1 + '&cat2Id=' + params.cat2Id + '&cat2Name=' + params.cat2Name + '&status=' + params.status + '&supplier1=' + params.supplier1); //添加
                        window.history.go(-1);
                    }
                } else {
                    WST.msg(json.msg, { icon: 2 });
...
//添加代码
function saleByPage_111(p, sta = 1) {
    $('#list').html('<tr><td colspan="11"><img src="' + WST.conf.ROOT + '/wstmart/home/view/default/img/loading.gif">正在加载数据...</td></tr>');
    var params = {};
...
function getCat(val) {
    ...
    $.post(WST.U('home/shopcats/listQuery'), { parentId: val }, function(data, textStatus) {
        ...
        if (json.status == 1 && json.list) {
            ...
            for (var i = 0; i < json.length; i++) {
                cat = json[i];
                //html.push("<option value='"+cat.catId+"'>"+cat.catName+"</option>");原代码
                //添加代码start
                if (cat2Id == cat.catId) {
                    html.push("<option value='" + cat.catId + "' selected='selected'>" + cat.catName + "</option>");
                } else {
...
//违规商品详情页跳转
function illegalDetail(id) {

    location.href = WST.U('home/goods/illegalDetail', { 'id': id });
}