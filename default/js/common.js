...
WST.initUserCenter = function(){
	...
	WST.getSysMessages('message,cart,userorder,complaint,cancelled');
	if(WST.conf.TIME_TASK=='1'){
		setInterval(function(){
			WST.getSysMessages('message,cart,userorder,complaint,cancelled');
		},1000); 
...
WST.cancelFavorite = function(obj,type,id,fId){
	...
	$.post(WST.U('home/favorites/cancel'),param,function(data,textStatus){
	    ...
	    if(json.status=='1'){
	       ...
	       $(obj).html('收藏'+str)[0].onclick = function(){
...
WST.getSysMessages = function(val){
	$.post(WST.U('home/index/getSysMessages'),{tasks:val},function(data){
		var json = WST.toJson(data);
		if(json.message){
			var num = parseInt(json.message.num) >99 ? "99+" : json.message.num;
			$('#wst-user-messages').html(num);
			if(parseInt(json.message.num,10)>0){
				...
				if($('#mId_'+json.message.id)[0])$('#mId_'+json.message.id).addClass('wst-msg-tips-box').html(num);
			}else{
        ...
        if(json.complaint){
			var num = parseInt(json.complaint.num) >99 ? "99+" : json.complaint.num;
            $('#wst-user-messages').html(num);
            if(parseInt(json.complaint.num,10)>0){
                $('#wst-user-messages').css('color','red');
                if($('.j-message-count')[0])$('.j-message-count').show().html(json.complaint.num);
                if($('#mId_'+json.complaint.id)[0])$('#mId_'+json.complaint.id).addClass('wst-msg-tips-box').html(num);
            }else{
                $('#wst-user-messages').css('color','#666');
                if($('.j-message-count')[0])$('.j-message-count').hide();
                if($('#mId_'+json.complaint.id)[0])$('#mId_'+json.complaint.id).removeClass('wst-msg-tips-box').html('');
            }
        }
        if(json.cancelled){
			var num = parseInt(json.cancelled.num) >99 ? "99+" : json.cancelled.num;
            $('#wst-user-messages').html(num);
            if(parseInt(json.cancelled.num,10)>0){
                $('#wst-user-messages').css('color','red');
                if($('.j-message-count')[0])$('.j-message-count').show().html(json.cancelled.num);
                if($('#mId_'+json.cancelled.id)[0])$('#mId_'+json.cancelled.id).addClass('wst-msg-tips-box').html(num);
            }else{
                $('#wst-user-messages').css('color','#666');
                if($('.j-message-count')[0])$('.j-message-count').hide();
                if($('#mId_'+json.cancelled.id)[0])$('#mId_'+json.cancelled.id).removeClass('wst-msg-tips-box').html('');
            }
        }
		if(json.cart){
			$('#goodsTotalNum').html(json.cart.goods);
			if(json.cart.goods>0){
				 var num = parseInt(json.cart.goods) >99 ? "99+" : json.cart.goods;
			     if($('.j-cart-count')[0])$('.j-cart-count').show().html(num);
			}else{
			...
			for(var key in json.userorder){
				if($('#mId_'+key)[0]){
					if(json.userorder[key]!='0'){
						var num = parseInt(json.userorder[key]) >99 ? "99+" :json.userorder[key];
					    $('#mId_'+key).addClass('wst-msg-tips-box').html(num);
					}else{
		...
		if(json.complaint){
			for(var key in json.complaint){
				if($('#mId_'+key)[0]){
					if(json.complaint[key]!='0'){
						var num = parseInt(json.complaint[key]) >99 ? "99+" :json.complaint[key];
					    $('#mId_'+key).addClass('wst-msg-tips-box').html(num);
					}else{
						$('#mId_'+key).removeClass('wst-msg-tips-box').html('');
					}
			...
			for(var key in json.shoporder){
				if($('#mId_'+key)[0]){
					if(json.shoporder[key]!='0'){
						var num = parseInt(json.shoporder[key]) >99 ? "99+" :json.shoporder[key];
					    $('#mId_'+key).addClass('wst-msg-tips-box').html(num);
					}else{
						$('#mId_'+key).removeClass('wst-msg-tips-box').html('');
					}
