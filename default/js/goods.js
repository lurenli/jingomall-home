$(function(){
	...
	$('.spec .j-option').click(function(){
		...
		checkGoodsStock();
		$("#buyNum").attr('data-max',$("#goods-stock").html());
	});
	fixedbar();
...
function checkGoodsStock(){
	...
	if(stock == 0){
		$('#buyBtn').addClass("un-buy").attr({'disabled':'disabled','href':'javascript:void(0)'}).click(function(){WST.msg("该商品库存为零，暂不能购买",{icon:2});});
		$('#goods-stock').css({'color':'#e60b49'});
		$('.pu').addClass('wst-tab_right_shopping1');
	}else{
        $('#buyBtn').unbind().removeClass("un-buy").removeAttr('disabled').attr('href','javascript:fastOrder("#buyNum")');
        $('#goods-stock').css({'color':'#585858'});
        $('.pu').removeClass('wst-tab_right_shopping1');
	}
...
function imagesMove(opts){
	...
	var moveLength = (scrollItems.eq(0).width()+10) * moveNum; //计算每次移动的长度
	var countLength = (scrollItems.length - viewNum) * (scrollItems.eq(0).width()+10); //计算总长度,总个数*单个长度
...
function fastOrder(iptId){
    ...
    $.post(WST.U('home/carts/sessionFastOrder'),{goodsId:goodsInfo.id,goodsSpecId:goodsSpecId,buyNum:buyNum,rnd:Math.random()},function(data,textStatus){
        var json = WST.toJson(data);
        // console.log(json);
        if(json.status==-1){
            WST.msg(json.msg,{icon:2});
        }else{
            location.href=WST.U('home/carts/fastOrder');
