function waitPayByPage(p){
	...
	$.post(WST.U('home/orders/waitPayByPage'),params,function(data,textStatus){
	    ...
	    if(json.status==1){
	    	json = json.data;
	    	//添加代码start
		      json.Rows['MemberStatus']=json.MemberStatus;
		      //添加代码end
...
//提醒卖家发货
function toRemind(id){
    var ll = WST.load({msg:'请稍等...'});
    $.post(WST.U('home/orders/shopSendMessage'),{orderId:id},function(data){
        var json = WST.toJson(data);
        if(json.code < 1 ){
...
function return_goods(id){
    // var retreat = WST.load({msg:'正在加载信息，请稍候...'});
    $.post(WST.U('home/order_sales_return/return_goods'),{id:id},function(data) {
        ...
        var w =WST.open({
            ...
            yes:function(index,layero){
                var params1 = {};
                params1.reason = $.trim($('#reason1').val());
                params1.content = $.trim($('#content1').val());
                params1.money = $.trim($('#money').val());
                params1.id = id;
                if(params1.money<0){
                ...
                var img = [];
                $('.complain_pic1').each(function(){
                    img.push($(this).attr('v'));
                });
                params1.returnAnnex = img.join(',');
                console.log(params1);

                if(params1.reason==10000 && params1.content==''){
                    WST.msg('请输入退货原因',{icon:2});
                    return;
                }
                $.post(WST.U('home/order_sales_return/saveSalesReturn'),params1,function(data,textStatus){
                    // layer.close(loading);
                    console.log(data);
                    ...
                    if(json.status==1){
                        WST.msg('您的退货信息已提交，请留意信息回复', {icon: 6},function(){
                            location.href = WST.U('home/orders/finish');
                        });
                    }else{
                        WST.msg(json.msg,{icon:2});
                    }
        ...
        callback:function(f,file){
            ...
            if(json.status==1){
                var tdiv = $("<div style='width:75px;float:left;margin-right:5px;'>"+
                    "<img class='complain_pic1"+"' width='75' height='75' src='"+WST.conf.ROOT+"/"+json.savePath+json.thumb+"' v='"+json.savePath+json.name+"'></div>");
                var btn = $('<div style="position:relative;top:-90px;left:60px;cursor:pointer;" ><img src="'+WST.conf.ROOT+'/wstmart/home/View/default/img/seller_icon_error.png"></div>');
                tdiv.append(btn);
                $('#picBox').append(tdiv);
...
function refund(id,src,type){
    var ll = WST.load({msg:'正在加载信息，请稍候...'});
	$.post(WST.U('home/orders/toRefund'),{id:id,type:type},function(data){
		layer.close(ll);
		var w = WST.open({
			    type: 1,
			    title:"申请退款",
...
function paymentOrderByPage(p){
    $('#loading').show();
    var params = {};
    params = WST.getParams('.u-query');
    params.key = $.trim($('#key').val());
    params.page = p;
    $.post(WST.U('home/orders/paymentOrderByPage'),params,function(data,textStatus){
        $('#loading').hide();
        var json = WST.toJson(data);
        $('.j-order-row').remove();
        if(json.status==1){
            ...
            //添加代码end
            if(params.page>json.TotalPage && json.TotalPage >0){
                paymentOrderByPage(json.TotalPage);
                return;
            }
            var gettpl = document.getElementById('tblist').innerHTML;
            laytpl(gettpl).render(json.Rows, function(html){
